...
#import "Mini Cooper Club App-Bridging-Header.h"
...
-(void)setTextView
{
    ...
    [attributedString addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Helvetica" size:17.0] range:NSMakeRange(0, messageText.length)];
    
    if (_message.name != nil) {
        
        messageText = @"";
        messageText = [NSString stringWithFormat:@"%@: \n%@", _message.name, _message.text];
        attributedString = [[NSMutableAttributedString alloc] initWithString:messageText];
        [attributedString addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Helvetica" size:17.0] range:NSMakeRange(0, messageText.length)];
        [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor blueColor] range:NSMakeRange(0, _message.name.length + 1)];
